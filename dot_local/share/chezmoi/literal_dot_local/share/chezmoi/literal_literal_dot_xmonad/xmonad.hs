...
myXPConfig = def { font = "xft:JetBrainsMono Nerd Font Mono:regular:size=9:antialias=true:hinting=true"
                 , bgColor = "#282c34"
                 , fgColor = "#bbc2cf"
                 , bgHLight = "#5b6268"
                 , fgHLight = "#ff6c6b"
                 , borderColor = "#ecbe7b"
                 , promptBorderWidth = 2
                 , position = CenteredAt (89 % 90) (59 % 60)
                 -- , position = Bottom
...
commands = fromList [ ("tt", spawn $ "urxvt")
                    , ("kk", spawn $ "kitty")
                    , ("ee", spawn $ "emacs")
                    -- , ("ff", sendMessage (Toggle NBFULL))
                    , ("dd", spawn "dmenu_run")
                    , ("ff", spawn "pcmanfm --new-win")
                    , ("qq", kill)
                    , ("lf", sendMessage FirstLayout)
                    , ("ln", sendMessage NextLayout)
                    , ("qQ", io (exitWith ExitSuccess))
                    , ("am", spawn "pamixer --toggle-mute")
                    , ("af", spawn "audacious --fwd")
                    , ("ar", spawn "audacious --rew")
                    , ("ap", spawn "audacious --play-pause")
                    , ("as", spawn "audacious --stop")
                    , ("qx", spawn "xkill")
                    , ("xl", spawn "xscreensaver-command -lock")
                    , ("ss", spawn "scrot '%Y-%m-%d_$wx$h_scrot.png' -e 'mv $f ~/Pictures/screenshots/'")
                    , ("sS", spawn "flameshot gui")
                    -- , ("xt", namedScratchpadAction myScratchpads "terminal")
                    -- , ("xa", namedScratchpadAction myScratchpads "audacious")
                    ]

runCommand :: String -> X ()
runCommand requestedCmd =
  case M.lookup requestedCmd commands of
    Nothing -> refresh --in future add error message
    Just (commandToExec) -> commandToExec

listeningPrompt :: X ()
listeningPrompt = inputPromptWithCompl myXPConfig "Listening" (mkComplFunFromList (M.keys commands)) ?+ runCommand
